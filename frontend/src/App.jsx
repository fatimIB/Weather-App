...
import History from "./components/WeatherHistory"
import WeatherForm from "./components/WeatherForm"
import ExportButton from "./components/ExportButton"
import EditForm from "./components/EditForm"
import SavedForecast from "./components/WeatherDetails"
import DeleteModal from "./components/DeleteModal"
import LocationExplore from "./components/LocationExplore"
...
function App() {
  ...
  const viewRecord = async (id) => {
    const data = await fetchWeatherDetails(id)
    setWeather(null)
  ...
  const confirmDelete = async () => {
    await removeWeather(deleteId)
    setDeleteId(null)
    getHistory()
    setUpdateMessage("Weather record deleted successfully")
    setTimeout(() => {
      setUpdateMessage("")
    }, 3000)
  }
  ...
  const editRecord = (record) => {
    ...
    setEditStart(record.start)
    setEditEnd(record.end)

  }
  const [updateMessage, setUpdateMessage] = useState("")
  const updateRecord = async () => {
    const data = await editWeather(
      editingRecord.id,
      {
  ...
  return (
    <div>
      ...
      />
      {
        ...
        <>
          <Forecast
            location={weather?.location || selectedWeather?.location}
      ...
      <EditForm
        ...
        cancelEdit={() => setEditingRecord(null)}
      />
      {
        showHistory &&
        <History
